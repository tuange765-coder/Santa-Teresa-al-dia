...
# ============================================
# CONEXION A BASE DE DATOS (CON FALLBACK A SQLITE)
# ============================================
def init_connection():
    try:
        # Intentar conectar a PostgreSQL si hay secrets
        ...
        st.warning(f"No se pudo conectar a PostgreSQL: {e}")
    
    # Fallback a SQLite local
    try:
        st.info("📁 Usando base de datos local SQLite")
        # Crear conexión SQLite
        sqlite_path = "santateresa.db"
        sqlite_conn = sqlite3.connect(sqlite_path, check_same_thread=False)
        return sqlite_conn, "sqlite"
    except Exception as e:
        st.error(f"Error crítico: No se pudo conectar a ninguna base de datos: {e}")
        st.stop()

conn, db_type = init_connection()

# ============================================
# FUNCIONES PARA EJECUTAR CONSULTAS SEGUN TIPO DE DB
# ============================================
...
def crear_tablas_si_no_existen():
    try:
        if db_type == "postgresql":
            ...
            cursor.execute("SELECT COUNT(*) FROM visitas WHERE id = 1")
            if cursor.fetchone()[0] == 0:
                cursor.execute("INSERT INTO visitas (id, conteo) VALUES (1, 1500)")
            
            cursor.execute("SELECT COUNT(*) FROM configuracion WHERE id = 1")
            if cursor.fetchone()[0] == 0:
                cursor.execute("INSERT INTO configuracion (id, logo_url, dolar) VALUES (1, NULL, 489.55)")
            
            # Datos iniciales
            cursor.execute("SELECT COUNT(*) FROM cronicas")
            if cursor.fetchone()[0] == 0:
                cronicas_iniciales = [
                ...
                for c in cronicas_iniciales:
                    cursor.execute("INSERT INTO cronicas (titulo, contenido, autor, fecha, lugar, estado) VALUES (?, ?, ?, ?, ?, ?)", c)
            
            cursor.execute("SELECT COUNT(*) FROM reflexiones")
            if cursor.fetchone()[0] == 0:
                cursor.execute("""
            ...
            if cursor.fetchone()[0] == 0:
                ...
                for n in noticias_iniciales:
                    cursor.execute("INSERT INTO noticias (titulo, categoria, contenido, fecha, autor) VALUES (?, ?, ?, ?, ?)", n)
...
def mostrar_video(video_data, formato):
    try:
        ...
        with open(tmp_path, "rb") as f:
            video_bytes_read = f.read()
        
        video_base64 = base64.b64encode(video_bytes_read).decode()
        
        st.markdown(f"""
        <div style="display: flex; justify-content: center;">
            <video width="50%" controls volume="0.1">
                <source src="data:video/{formato};base64,{video_base64}" type="video/{formato}">
...
def mostrar_audio(audio_data, formato):
    try:
        ...
        audio_base64 = base64.b64encode(audio_bytes_read).decode()
        
        st.markdown(f"""
        <div style="display: flex; justify-content: center;">
            <audio controls style="width: 50%;" volume="0.1">
                <source src="data:audio/{formato};base64,{audio_base64}" type="audio/{formato}">
...
def get_dolar():
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT dolar FROM configuracion WHERE id = 1")
...
def actualizar_dolar_manual(nuevo_valor):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("UPDATE configuracion SET dolar = ? WHERE id = 1", (float(nuevo_valor),))
            conn.commit()
        return True
    except Exception:
        return False
...
def actualizar_visitas():
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("UPDATE visitas SET conteo = conteo + 1 WHERE id = 1")
            conn.commit()
    except:
        pass
...
def get_visitas():
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT conteo FROM visitas WHERE id = 1")
            result = cursor.fetchone()
            if result:
                return int(result[0])
        return 1500
...
def get_logo():
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT logo_url FROM configuracion WHERE id = 1")
            result = cursor.fetchone()
            if result and result[0]:
                return result[0]
        return None
...
def save_logo(url):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("UPDATE configuracion SET logo_url = ? WHERE id = 1", (url,))
            conn.commit()
        return True
    except:
        return False
    ...
    try:
        ...
        if db_type == "postgresql":
            with conn.session as s:
                ...
                """), {"t": titulo, "c": categoria, "cont": contenido, "i": img_url, "f": fecha})
                s.commit()
        else:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO noticias (titulo, categoria, contenido, imagen_url, fecha, autor)
                VALUES (?, ?, ?, ?, ?, 'Admin')
            """, (titulo, categoria, contenido, img_url, fecha))
            conn.commit()
        return True
    except:
        return False
...
def get_noticias(categoria=None):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            if categoria and categoria != "Todas":
                cursor.execute("SELECT * FROM noticias WHERE categoria = ? ORDER BY id DESC", (categoria,))
            else:
...
def delete_noticia(id_):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("DELETE FROM noticias WHERE id = ?", (id_,))
            conn.commit()
        return True
    except:
        return False
    ...
    try:
        ...
        if db_type == "postgresql":
            with conn.session as s:
                ...
                """), {"n": nombre, "c": categoria, "r": resena, "i": img_url, "d": direccion, "t": telefono, "h": horario, "f": fecha})
                s.commit()
        else:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO negocios (nombre, categoria, resena, imagen_url, direccion, telefono, horario, fecha)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (nombre, categoria, resena, img_url, direccion, telefono, horario, fecha))
            conn.commit()
        return True
    except:
        return False
...
def get_negocios():
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM negocios ORDER BY id DESC")
            columns = [description[0] for description in cursor.description]
            data = cursor.fetchall()
            return pd.DataFrame(data, columns=columns)
    except:
...
def delete_negocio(id_):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("DELETE FROM negocios WHERE id = ?", (id_,))
            conn.commit()
        return True
    except:
        return False
...
def add_reflexion(titulo, contenido, versiculo):
    try:
        ...
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("UPDATE reflexiones SET activo = 0")
            cursor.execute("""
                INSERT INTO reflexiones (titulo, contenido, versiculo, autor, fecha, activo)
                VALUES (?, ?, ?, 'Admin', ?, 1)
            """, (titulo, contenido, versiculo, fecha))
            conn.commit()
        return True
    except:
        return False
...
def get_reflexion_activa():
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM reflexiones WHERE activo = 1 LIMIT 1")
            columns = [description[0] for description in cursor.description]
...
def get_reflexiones():
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM reflexiones ORDER BY id DESC")
            columns = [description[0] for description in cursor.description]
            data = cursor.fetchall()
            return pd.DataFrame(data, columns=columns)
    except:
...
def delete_reflexion(id_):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("DELETE FROM reflexiones WHERE id = ?", (id_,))
            conn.commit()
        return True
    except:
        return False
    ...
    try:
        ...
        if db_type == "postgresql":
            with conn.session as s:
                ...
                """), {"t": titulo, "c": contenido, "f": fecha, "l": lugar, "e": estado})
                s.commit()
        else:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO cronicas (titulo, contenido, autor, fecha, lugar, estado)
                VALUES (?, ?, 'Admin', ?, ?, ?)
            """, (titulo, contenido, fecha, lugar, estado))
            conn.commit()
        return True
    except:
        return False
...
def get_cronicas(estado=None):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            if estado and estado != "Todos":
                cursor.execute("SELECT * FROM cronicas WHERE estado = ? ORDER BY id DESC", (estado,))
            else:
                cursor.execute("SELECT * FROM cronicas ORDER BY id DESC")
            columns = [description[0] for description in cursor.description]
            data = cursor.fetchall()
            return pd.DataFrame(data, columns=columns)
    except:
...
def delete_cronica(id_):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("DELETE FROM cronicas WHERE id = ?", (id_,))
            conn.commit()
        return True
    except:
        return False
            ...
            if db_type == "postgresql":
                with conn.session as s:
                    s.execute(text("""
                        ...
                        VALUES (:t, :v, :fmt, :f)
                    """), {"t": titulo, "v": video_data, "fmt": formato, "f": fecha})
                    s.commit()
            else:
                cursor = conn.cursor()
                cursor.execute("""
                    INSERT INTO videos (titulo, video_data, formato, fecha)
                    VALUES (?, ?, ?, ?)
                """, (titulo, video_data, formato, fecha))
                conn.commit()
            return True
        return False
    except Exception:
...
def delete_video(id_):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("DELETE FROM videos WHERE id = ?", (id_,))
            conn.commit()
        return True
    except:
        return False
            ...
            if db_type == "postgresql":
                with conn.session as s:
                    s.execute(text("""
                        ...
                        VALUES (:t, :a, :fmt, :f)
                    """), {"t": titulo, "a": audio_data, "fmt": formato, "f": fecha})
                    s.commit()
            else:
                cursor = conn.cursor()
                cursor.execute("""
                    INSERT INTO musicas (titulo, audio_data, formato, fecha)
                    VALUES (?, ?, ?, ?)
                """, (titulo, audio_data, formato, fecha))
                conn.commit()
            return True
        return False
    except Exception:
...
def get_musicas():
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM musicas ORDER BY id DESC")
            columns = [description[0] for description in cursor.description]
            data = cursor.fetchall()
            return pd.DataFrame(data, columns=columns)
    except:
...
def delete_musica(id_):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("DELETE FROM musicas WHERE id = ?", (id_,))
            conn.commit()
        return True
    except:
        return False
    ...
    try:
        ...
        if db_type == "postgresql":
            with conn.session as s:
                ...
                """), {"d": denunciante or "Anonimo", "t": titulo, "desc": descripcion, "u": ubicacion, "f": fecha})
                s.commit()
        else:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO denuncias (denunciante, titulo, descripcion, ubicacion, fecha, estatus)
                VALUES (?, ?, ?, ?, ?, 'Pendiente')
            """, (denunciante or "Anonimo", titulo, descripcion, ubicacion, fecha))
            conn.commit()
        return True
    except:
        return False
...
def update_denuncia_status(id_, status):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("UPDATE denuncias SET estatus = ? WHERE id = ?", (status, id_))
            conn.commit()
        return True
    except:
        return False
...
def delete_denuncia(id_):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("DELETE FROM denuncias WHERE id = ?", (id_,))
            conn.commit()
        return True
    except:
        return False
    ...
    try:
        ...
        if db_type == "postgresql":
            ...
            cursor.execute("""
                INSERT INTO opiniones (usuario, comentario, calificacion, fecha, aprobada)
                VALUES (?, ?, ?, ?, 0)
            """, (usuario, comentario, calificacion, fecha))
            conn.commit()
        return True
    except:
        return False
...
def get_opiniones(aprobadas=True):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            if aprobadas:
                cursor.execute("SELECT * FROM opiniones WHERE aprobada = 1 ORDER BY id DESC")
            else:
                cursor.execute("SELECT * FROM opiniones ORDER BY id DESC")
            columns = [description[0] for description in cursor.description]
            data = cursor.fetchall()
            return pd.DataFrame(data, columns=columns)
    except:
...
def approve_opinion(id_):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("UPDATE opiniones SET aprobada = 1 WHERE id = ?", (id_,))
            conn.commit()
        return True
    except:
        return False
...
def delete_opinion(id_):
    try:
        if db_type == "postgresql":
            ...
            cursor = conn.cursor()
            cursor.execute("DELETE FROM opiniones WHERE id = ?", (id_,))
            conn.commit()
        return True
    except:
        return False
...
# ============================================
# ESTILOS PRINCIPALES CON EQUALIZADOR
# ============================================
st.markdown("""
<style>
.stApp {
...
.equalizer .bar:nth-child(2) { animation-delay: 0.2s; height: 30px; }
.equalizer .bar:nth-child(3) { animation-delay: 0.4s; height: 25px; }
.equalizer .bar:nth-child(4) { animation-delay: 0.6s; height: 35px; }
.equalizer .bar:nth-child(5) { animation-delay: 0.8s; height: 20px; }
.equalizer .bar:nth-child(6) { animation-delay: 1.0s; height: 28px; }
.equalizer .bar:nth-child(7) { animation-delay: 1.2s; height: 18px; }
.equalizer .bar:nth-child(8) { animation-delay: 1.4s; height: 32px; }
@keyframes equalize {
    0%, 100% { height: 15px; }
    50% { height: 40px; }
}
...
with st.sidebar:
    ...
    if es_admin:
        ...
        admin_opt = st.radio("Seleccionar módulo:", [
            ...
            "🎬 Videos", "🎵 Música", "🗣️ Haz que tu voz se escuche (Denuncias)", "💬 Opiniones", "⚙️ Configuración"
        ])
...
# ============================================
menu_tabs = st.tabs(["🏠 Portada", "📰 Noticias", "📍 Donde ir - Donde comprar", "💭 Reflexiones", "📜 Crónicas", "🎬 Multimedia", "🗣️ Haz que tu voz se escuche", "💬 Opiniones", "📅 Efemérides Médicas"])
...
with menu_tabs[0]:
    ...
    # Sección de Videos destacados en la portada
    ...
    if not videos.empty:
        for _, v in videos.head(5).iterrows():
            with st.container():
                st.markdown(f"""
                <div style="background: rgba(0,0,0,0.3); border-radius: 15px; padding: 15px; margin: 10px 0; border-left: 5px solid #FFD700;">
                    <h4 style="color: #FFD700; margin: 0 0 10px 0;">🎬 {v['titulo']}</h4>
                </div>
                """, unsafe_allow_html=True)
                mostrar_video(v['video_data'], v['formato'])
                st.caption(f"📅 Agregado: {v['fecha']}")
                st.markdown("---")
        if len(videos) > 5:
            st.info(f"🎬 Hay {len(videos)} videos en total. Ve a la pestaña 'Multimedia > Videos' para ver todos.")
    else:
        st.info("🎬 No hay videos disponibles. Usa el panel de administración para agregar videos.")
    
    st.markdown("---")
    
    # Sección de Música destacada en la portada
    st.markdown("### 🎵 Música Destacada")
    musicas = get_musicas()
    if not musicas.empty:
        for _, m in musicas.head(5).iterrows():
            with st.container():
                st.markdown(f"""
                <div style="background: rgba(0,0,0,0.3); border-radius: 15px; padding: 15px; margin: 10px 0; border-left: 5px solid #FFD700;">
                    <h4 style="color: #FFD700; margin: 0 0 10px 0;">🎵 {m['titulo']}</h4>
                </div>
                """, unsafe_allow_html=True)
                # Equalizador animado
                st.markdown("""
                ...
                st.markdown("---")
        if len(musicas) > 5:
            st.info(f"🎶 Hay {len(musicas)} canciones en total. Ve a la pestaña 'Multimedia > Música' para ver todas.")
    else:
        st.info("🎵 No hay música disponible. Usa el panel de administración para agregar canciones.")
...
with menu_tabs[5]:
    ...
    with tab_vid:
        ...
        videos = get_videos()
        if not videos.empty:
            st.success(f"📀 **Total de videos disponibles:** {len(videos)}")
    ...
    with tab_mus:
        st.markdown("### 🎵 Biblioteca de Música")
        musicas = get_musicas()
        if not musicas.empty:
            st.success(f"📀 **Total de canciones disponibles:** {len(musicas)}")
...
# --- TAB 6: HAZ QUE TU VOZ SE ESCUCHE ---
with menu_tabs[6]:
    st.title("🗣️ Haz que tu voz se escuche")
    st.markdown("### Denuncias Ciudadanas")
...
with menu_tabs[8]:
    ...
    with col_ven:
        ...
        if efemeride_hoy_ven:
            st.success(f"### 🎉 ¡EFEMÉRIDE DEL DÍA!\n**{fecha_actual_str}**\n{efemeride_hoy_ven}")
        else:
...
if st.session_state.get('es_admin', False):
    ...
    if "📰 Noticias" in admin_opt:
        ...
        st.info("📌 Sube archivos de video en formato MP4, AVI, MOV o MKV (máximo 50MB)")
        with st.form("fvid"):
        ...
        st.markdown("---")
        st.markdown("### 📋 Lista de videos completa")
        videos = get_videos()
        if not videos.empty:
            st.success(f"🎬 **Total de videos:** {len(videos)}")
            for _, v in videos.iterrows():
            ...
            st.info("No hay videos agregados. Usa el formulario de arriba para subir videos.")
    
    elif "🎵 Música" in admin_opt:
        st.subheader("🎵 Gestionar Música")
        st.info("📌 Sube archivos de audio en formato MP3, WAV o OGG (máximo 20MB)")
        with st.form("fmus"):
        ...
        st.markdown("---")
        st.markdown("### 📋 Lista de reproducción completa")
        musicas = get_musicas()
        if not musicas.empty:
            st.success(f"🎵 **Total de canciones:** {len(musicas)}")
            for _, m in musicas.iterrows():
            ...
            st.info("No hay música agregada. Usa el formulario de arriba para subir canciones.")
    
    elif "🗣️ Haz que tu voz se escuche (Denuncias)" in admin_opt:
        st.subheader("🗣️ Gestionar Denuncias Ciudadanas")
