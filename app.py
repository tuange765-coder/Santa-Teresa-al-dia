...
# ============================================
# DETECTAR DISPOSITIVO MOVIL Y OCULTAR OPCIONES DE DESARROLLO
# ============================================
def is_mobile():
    try:
...
# Ocultar elementos de desarrollo en móvil
if es_movil:
    st.markdown("""
    <style>
...
# ============================================
# OCULTAR BOTONES DE DESARROLLO EN TODOS LOS DISPOSITIVOS
# ============================================
st.markdown("""
<style>
...
.uploadedFileName {display: none !important;}
.st-emotion-cache-1y4p8pa {display: none !important;}
</style>
""", unsafe_allow_html=True)

# ============================================
# FUNCION PARA OBTENER URL DE LA APP (SIN DATOS PERSONALES)
# ============================================
def get_app_url():
    try:
        ...
        pass
    return "https://santa-teresa.streamlit.app"
...
def init_connection():
    try:
        if "DATABASE_URL" in st.secrets:
            ...
            st.error("Error de configuracion. Contacte al administrador.")
            st.stop()
        test_query = conn.query("SELECT 1 as test", ttl=0)
        if test_query.empty:
            st.error("Error de conexion. Contacte al administrador.")
            st.stop()
        return conn
    except Exception:
        st.error("Error de conexion. Contacte al administrador.")
    ...
    if file:
        try:
            ...
            if len(bytes_data) > 50 * 1024 * 1024:
                st.error("El video es muy grande (maximo 50 MB)")
                return None
    ...
    if file:
        try:
            ...
            if len(bytes_data) > 20 * 1024 * 1024:
                st.error("El audio es muy grande (maximo 20 MB)")
                return None
            return base64.b64encode(bytes_data).decode()
        except Exception:
            return None
...
# ============================================
# BOTONES DE COMPARTIR EN LA PORTADA
# ============================================
col_share1, col_share2, col_share3 = st.columns([1, 1, 2])
app_url = get_app_url()
whatsapp_url = f"https://api.whatsapp.com/send?text=Santa Teresa al Dia - Informacion, Cultura y Fe - {app_url}"
...
with st.sidebar:
    ...
    # Panel de Administracion - SOLO CON CONTRASEÑA
    if not es_movil:
        ...
        if not opiniones.empty:
            for _, op in opiniones.iterrows():
                ...
                <div style="background: rgba(0,0,0,0.3); padding: 15px; border-radius: 10px; margin-bottom: 10px;">
                    <strong>{op['usuario']}</strong> {estrellas_texto}<br>
                    "{op['comentario']}"<br>
...
if 'es_admin' in locals() and es_admin:
    ...
    # --- ADMIN: NEGOCIOS ---
        ...
        with st.form("form_negocio"):
            ...
            resena = st.text_area("Resena", height=100)
    ...
    # --- ADMIN: VIDEOS ---
        ...
        with st.form("form_video"):
            ...
            archivo_video = st.file_uploader("Seleccionar video", type=["mp4", "avi", "mov", "mkv"])
            st.info("Tamano maximo: 50 MB")
    ...
    # --- ADMIN: MUSICA ---
        ...
        with st.form("form_musica"):
            st.subheader("Subir Musica")
            titulo = st.text_input("Titulo de la Cancion")
            archivo_audio = st.file_uploader("Seleccionar audio", type=["mp3", "wav", "ogg"])
            st.info("Tamano maximo: 20 MB")
    ...
    # --- ADMIN: OPINIONES ---
        ...
        if not opiniones_aprobadas.empty:
            for _, op in opiniones_aprobadas.iterrows():
                with st.expander(f"{op['usuario']} - {op['fecha']}"):
                    st.write(op['comentario'])
                    if st.button("Eliminar", key=f"del_aprob_{op['id']}"):
                        delete_opinion(op['id'])
                        st.rerun()
        else:
    ...
    # --- ADMIN: CONFIGURACION ---
    elif admin_option == "Configuracion":
        ...
        with col2:
            ...
            nuevo_dolar = st.number_input("Nuevo valor", value=float(dolar), step=0.01)
