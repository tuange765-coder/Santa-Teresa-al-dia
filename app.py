...
def mostrar_video(video_data, formato):
    try:
        ...
        # Reproductor de video al 50% con volumen bajo
        with open(tmp_path, "rb") as f:
            video_bytes_read = f.read()
        
        video_base64 = base64.b64encode(video_bytes_read).decode()
        
        # HTML5 video con volumen bajo (0.1) y ancho 50%
        st.markdown(f"""
        <div style="display: flex; justify-content: center;">
            <video width="50%" controls autoplay volume="0.1">
...
def mostrar_audio(audio_data, formato):
    try:
        ...
        # Reproductor de audio al 50% con volumen bajo
        with open(tmp_path, "rb") as f:
            audio_bytes_read = f.read()
        
        audio_base64 = base64.b64encode(audio_bytes_read).decode()
        
        # HTML5 audio con volumen bajo (0.1)
        st.markdown(f"""
        <div style="display: flex; justify-content: center;">
            <audio controls style="width: 50%;" volume="0.1" autoplay>
                <source src="data:audio/{formato};base64,{audio_base64}" type="audio/{formato}">
                Tu navegador no soporta el elemento de audio.
            </audio>
        </div>
        <script>
            // Forzar volumen bajo después de cargar
...
# ============================================
# TAB 5: MULTIMEDIA - CON VIDEOS Y MUSICA AL 50% Y EQUALIZADOR
# ============================================
with menu_tabs[5]:
    st.title("🎬 Multimedia")
    ...
    with tab_vid:
        ...
        if not videos.empty:
            ...
            st.info("🎬 No hay videos disponibles. Usa el panel de administración para agregar videos.")
            st.info("📌 Para agregar videos: Barra lateral → Administración → Clave: Juan*316* → Selecciona 'Videos'")
    ...
    with tab_mus:
        ...
        if not musicas.empty:
            ...
            st.info("🎵 No hay música disponible. Usa el panel de administración para agregar canciones.")
            st.info("📌 Para agregar música: Barra lateral → Administración → Clave: Juan*316* → Selecciona 'Música'")
...
# --- TAB 6: HAZ QUE TU VOZ SE ESCUCHE (DENUNCIAS) ---
with menu_tabs[6]:
