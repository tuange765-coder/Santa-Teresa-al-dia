...
with col1:
    st.markdown(f'<a href="{whatsapp_url}" target="_blank"><div style="background:#25D366;padding:8px;border-radius:20px;text-align:center;"><span style="color:white;">WhatsApp</span></div></a>', unsafe_allow_html=True)

with col2:
    st.markdown(f'<a href="{facebook_url}" target="_blank"><div style="background:#1877F2;padding:8px;border-radius:20px;text-align:center;"><span style="color:white;">Facebook</span></div></a>', unsafe_allow_html=True)

with col3:
    if st.button("Instagram", key="btn_insta"):
        copy_to_clipboard_js(APP_URL)
        st.success("Enlace copiado!")

with col4:
    if st.button("Copiar enlace", key="btn_copy"):
        copy_to_clipboard_js(APP_URL)
        st.success("Enlace copiado!")
...
if menu == "Portada":
    ...
    tab1, tab2, tab3 = st.tabs(["Nacionales", "Internacionales", "Deportes"])
    
    with tab1:
        for _, n in get_noticias(categoria="Nacional").iterrows():
    ...
    with tab2:
        for _, n in get_noticias(categoria="Internacional").iterrows():
            with st.expander(f"{n['titulo']} - {n['fecha']}"):
                if n['imagen_url']:
                    st.image(n['imagen_url'], width=300)
                st.write(n['contenido'])
    ...
    with tab3:
        for _, n in get_noticias(categoria="Deportes").iterrows():
            with st.expander(f"{n['titulo']} - {n['fecha']}"):
                if n['imagen_url']:
                    st.image(n['imagen_url'], width=300)
                st.write(n['contenido'])
    ...
    for _, c in get_cronicas(estado_filtro if estado_filtro != "Todos" else None).iterrows():
