...
# ============================================
# OCULTAR SOLO ELEMENTOS DE STREAMLIT (NO EL CONTENIDO)
# ============================================
st.markdown("""
<style>
...
# ============================================
# FUNCION PARA OBTENER URL DE LA APP
# ============================================
def get_app_url():
    try:
        ...
        pass
    return "https://santa-teresa-al-dia.streamlit.app"
...
/* Estilo para los expansores de noticias y reflexiones */
...
# ============================================
# BOTONES DE COMPARTIR (AGREGADOS EN LA PORTADA)
# ============================================
col_share1, col_share2, col_share3 = st.columns([1, 1, 2])
app_url = get_app_url()
whatsapp_url = f"https://api.whatsapp.com/send?text=🌟 Santa Teresa al Dia - Informacion, Cultura y Fe para Nuestra Comunidad%0A%0A🔗 {app_url}"
...
with col_share1:
    ...
    <a href="{whatsapp_url}" target="_blank" style="text-decoration: none;">
        <div style="background: linear-gradient(135deg, #25D366, #128C7E); 
                    ...
                    border-radius: 20px; 
                    text-align: center;">
            <span style="color: white; font-weight: bold;">📲 Compartir WhatsApp</span>
        </div>
    </a>
    ''', unsafe_allow_html=True)
...
# --- PORTADA (MODIFICADA: NOTICIAS Y REFLEXIONES EN EXPANDER) ---
if menu == "🏠 Portada":
    st.title("🌟 Santa Teresa al Dia")
    
    # ============================================
    # SECCION DE NOTICIAS - TITULO CON EXPANDER
    # ============================================
    st.markdown("### 📰 Ultimas Noticias")
    noticias = get_noticias()
    if not noticias.empty:
        for _, n in noticias.head(10).iterrows():
            # Crear un expander para cada noticia - solo muestra el título al inicio
            with st.expander(f"📌 {n['titulo']} - {n['categoria']} ({n['fecha']})"):
                # Dentro del expander se muestra el contenido completo
                st.markdown(f"**Fecha:** {n['fecha']}")
                st.markdown(f"**Categoría:** {n['categoria']}")
                if n['imagen_url']:
                    st.image(n['imagen_url'], width=300)
                st.markdown(f"**Contenido:**")
                st.write(n['contenido'])
                st.caption(f"Publicado por: {n['autor']}")
    else:
    ...
    # ============================================
    # SECCION DE REFLEXIONES - TITULO CON EXPANDER
    # ============================================
    st.markdown("### 🙏 Reflexion del Dia")
    ref = get_reflexion_activa()
    if ref is not None:
        # Reflexión activa también en expander
        with st.expander(f"✨ {ref['titulo']} - {ref['fecha']} ✨"):
            st.markdown(f"**Autor:** {ref['autor']}")
            st.markdown(f"**Fecha:** {ref['fecha']}")
            st.markdown(f"**Contenido:**")
            st.write(ref['contenido'])
            st.markdown(f"**Versículo:** 📖 {ref['versiculo']}")
    else:
    ...
    # ============================================
    # REFLEXIONES ANTERIORES (ARCHIVO)
    # ============================================
    st.markdown("### 📜 Reflexiones Anteriores")
    reflexiones_anteriores = get_reflexiones()
    if not reflexiones_anteriores.empty:
        # Mostrar solo las últimas 5 reflexiones (excluyendo la activa si está)
        refs_mostrar = reflexiones_anteriores.head(10)
        for _, r in refs_mostrar.iterrows():
            with st.expander(f"📖 {r['titulo']} - {r['fecha']}"):
                st.markdown(f"**Autor:** {r['autor']}")
                st.markdown(f"**Contenido:**")
                st.write(r['contenido'])
                if r['versiculo']:
                    st.markdown(f"**Versículo:** 📖 {r['versiculo']}")
    else:
        st.info("No hay reflexiones anteriores disponibles")
    
    st.markdown("---")
    
    # ============================================
    # RECOMENDADOS (NEGOCIOS DESTACADOS)
    # ============================================
    ...
    if not negocios.empty:
        ...
        st.info("No hay negocios destacados aún")

# --- NOTICIAS (SECCION COMPLETA) ---
elif menu == "📰 Noticias":
    st.title("📰 Noticias")
...
# --- REFLEXIONES (SECCION COMPLETA) ---
    ...
    ref = get_reflexion_activa()
    if ref is not None:
        with st.expander(f"✨ REFLEXIÓN ACTIVA: {ref['titulo']} ✨", expanded=True):
            st.markdown(f"**Autor:** {ref['autor']}")
            st.markdown(f"**Fecha:** {ref['fecha']}")
            st.write(ref['contenido'])
            st.markdown(f"**Versículo:** 📖 {ref['versiculo']}")
